...
function parse( doc, clbk ) {
	...
	function onTextResolve( content ) {
		...
		for ( j = 0; j < o.length; j++ ) {
			...
			if ( isnan( c ) && keys.length === 1 ) {
				if ( row ) {
					debug( 'Transaction row: %s', JSON.stringify( row ) );
					results.push( row );
					row = null;
				}
				type = v[ 7 ];
			...
			row = {
				...
				'Daily Cash ($)': v[ 89 ],
				'Amount': v[ 111 ] || v[ 110 ] || v[ 109 ] || v[ 108 ] || v[ 107 ] // the amount can vary in its `x` position
			};
		}
	...
	*/
	function done( error ) {
		...
		debug( 'Raw transactions: %s', JSON.stringify( results ) );
